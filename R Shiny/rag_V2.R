...
initialize_rag <- function(
    store_path = "RAG/chunk_store.rds",
    file_paths = c(
      "Main RMDs/Project V1.Rmd",
...
# ---------------------------
# Main function for Shiny
# ---------------------------
run_rag <- function(question, context = NULL, store, hf_token = Sys.getenv("HUGGINGFACE_API_KEY"), k = 3) {
  if (hf_token == "") {
    stop("HUGGINGFACE_API_KEY is not set")
  }
